  ...
  return (
    <div className="space-y-8">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-xl lg:text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
              <BarChart3 className="w-6 h-6 text-slate-900" /> Operational Analytics & Longitudinal Trends
            </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-slate-50/90 border border-slate-200 shadow-xl">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">
                Day-of-Week Turnout Pattern (Avg Meals / Day)
              </h3>
              <p className="text-xs text-slate-500">Illustrates Friday evening & weekend student travel dip</p>
            </div>
        ...
        <div className="p-6 rounded-2xl bg-slate-50/90 border border-slate-200 shadow-xl">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500">Relative popularity index out of 100 based on student check-ins</p>
            </div>
            <span className="text-xs font-mono text-slate-900">Turnout %</span>
          </div>
            ...
            {menuData.map((item) => (
              <div key={item.menu} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-slate-700">{item.menu}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-900 font-bold">{item.avg_turnout}</span>
                    <span className="text-slate-500 font-mono text-[10px]">({item.demand_score}/100)</span>
                  </div>
                </div>
      ...
      <div className="p-6 rounded-2xl bg-slate-50/90 border border-slate-300/60 shadow-xl space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          ...
          <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-300">
        ...
        {modelEval && (
          <div className="space-y-4">
            {/* Honest message */}
            <div className="p-4 rounded-xl bg-slate-50/80 border border-indigo-300 flex items-start gap-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
              <div className="p-4 rounded-xl bg-slate-50/60 border border-slate-200">
                <span className="text-slate-500 block mb-1">Interim Mean Absolute Error (MAE)</span>
                <span className="text-xl font-bold font-mono text-indigo-500">{modelEval.interim_mae}</span>
                <span className="text-[10px] text-slate-500 block mt-1">Average deviation per meal slot</span>
              </div>
              <div className="p-4 rounded-xl bg-slate-50/60 border border-slate-200">
                <span className="text-slate-500 block mb-1">Interim R² Coefficient of Determination</span>
                <span className="text-xl font-bold font-mono text-slate-900">{modelEval.interim_r2}</span>
                <span className="text-[10px] text-slate-500 block mt-1">Synthetic training variance explained</span>
              </div>
              <div className="p-4 rounded-xl bg-slate-50/60 border border-slate-200">
                <span className="text-slate-500 block mb-1">Historical Feed Sessions Logged</span>
                <span className="text-xl font-bold font-mono text-slate-900">{modelEval.sample_records_count} meals</span>
                <span className="text-[10px] text-slate-500 block mt-1">Populated in SQLite demo database</span>
              </div>
            </div>
          </div>
        )}

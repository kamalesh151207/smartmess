  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          <Download className="w-4 h-4 text-slate-50" /> Export Audit Log
        </button>
      </div>
      ...
      <div className="rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between text-xs text-slate-400">
          <span>
            Showing page <strong className="text-slate-50">{page}</strong> of <strong className="text-slate-50">{totalPages}</strong> ({totalCount} total service records)
          </span>
              ...
              {loading ? (
                ...
                history.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-3 px-4 font-bold text-slate-50">{row.meal}</td>
                    <td className="py-3 px-4 max-w-xs truncate text-slate-400" title={row.menu}>
                      {row.menu}
                    </td>
                    <td className="py-3 px-4 text-center font-mono text-indigo-500">{row.predicted}</td>
                    <td className="py-3 px-4 text-center font-mono text-slate-200">{row.actual}</td>
                    <td className="py-3 px-4 text-center font-mono font-bold">
                      <span className={row.variance > 0 ? 'text-indigo-600' : 'text-slate-400'}>
                        {row.variance > 0 ? `+${row.variance}` : row.variance}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-center font-mono text-slate-50">{row.prepared}</td>
                    <td className="py-3 px-4 text-center font-mono text-slate-50 font-bold">{row.leftover}</td>
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-indigo-500 hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
        ...
        <div className="p-4 border-t border-slate-800 flex items-center justify-between text-xs">
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-slate-50 disabled:opacity-40 flex items-center gap-1 cursor-pointer"
          >
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-2xl relative">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-mono text-slate-400">{selectedRecord.date}</span>
            </div>

            <h3 className="text-lg font-bold text-slate-50 mb-2">{selectedRecord.menu}</h3>

            <div className="grid grid-cols-2 gap-3 my-4">
              <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                <span className="text-xs text-slate-400 block mb-1">AI Prediction</span>
                <span className="text-2xl font-bold text-indigo-500">{selectedRecord.predicted}</span>
                <span className="text-[10px] text-slate-400 block mt-0.5">Forecast portions</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                <span className="text-xs text-slate-400 block mb-1">Actual Consumed</span>
                <span className="text-2xl font-bold text-slate-50">{selectedRecord.actual}</span>
                <span className="text-[10px] text-slate-400 block mt-0.5">Turnout check-ins</span>
              </div>
            </div>

            <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-400 space-y-2 mb-6">
              <div className="flex justify-between">
                <span className="text-slate-400">Kitchen Prepared:</span>
                <strong className="text-slate-50">{selectedRecord.prepared} meals</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Food Leftover:</span>
                <strong className="text-indigo-600">{selectedRecord.leftover} portions</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Variance (Actual - Predicted):</span>
                <strong className={selectedRecord.variance > 0 ? 'text-indigo-600' : 'text-slate-400'}>
                  {selectedRecord.variance > 0 ? `+${selectedRecord.variance}` : selectedRecord.variance}
                </strong>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Service Evaluation:</span>

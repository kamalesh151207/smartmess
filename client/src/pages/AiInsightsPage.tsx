  ...
  if (loading || !data) {
    return (
      <div className="space-y-6 animate-pulse">
        <div className="h-40 rounded-2xl bg-slate-50/60 border border-slate-200" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="h-64 rounded-2xl bg-slate-50/60 border border-slate-200" />
          <div className="h-64 rounded-2xl bg-slate-50/60 border border-slate-200" />
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Pattern Detection */}
        <div className="lg:col-span-7 p-6 rounded-2xl bg-slate-50/90 border border-slate-200 shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-slate-900" /> Observed Historical Patterns
            </h3>
            <span className="text-[10px] text-slate-500 font-mono">3 Active Detections</span>
          </div>
          ...
          <div className="space-y-3">
            {data.pattern_detection.map((p: any) => (
              ...
              >
                ...
                <div className="p-2.5 rounded-lg bg-blue-100/40 border border-blue-200/50 text-[11px] text-slate-700 flex items-start gap-2">
                  <strong className="text-slate-900 flex-shrink-0">Action:</strong>
                  <span>{p.action}</span>
                </div>
              </div>
            ))}
        ...
        <div className="lg:col-span-5 p-6 rounded-2xl bg-slate-50/90 border border-slate-200 shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-4">
              ...
              <span className="text-[10px] font-mono text-slate-500">RandomForest</span>
            </div>
              ...
              {data.model_signals.map((sig: any) => (
                <div key={sig.feature} className="space-y-1 text-xs">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-slate-500">{sig.feature}</span>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-slate-900 font-bold">{sig.percentage}%</span>
                    </div>
                  </div>
          ...
          <div className="mt-6 p-3 rounded-xl bg-slate-50/60 border border-slate-200 text-[11px] text-slate-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-slate-50/90 border border-slate-200 shadow-xl space-y-4">
          ...
          <div className="space-y-3">
            {data.recommendations.map((r: any, idx: number) => (
              <div
                key={idx}
                className="p-3.5 rounded-xl bg-slate-50/80 border border-slate-200 flex items-start gap-3"
              >
                <div
                ...
                <div>
                  <span className="text-xs font-bold text-slate-900 block mb-0.5">{r.target}</span>
                  <p className="text-xs text-slate-500 leading-relaxed">{r.recommendation}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-6 rounded-2xl bg-slate-50/90 border border-slate-200 shadow-xl space-y-4">
          <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-slate-900" /> Active Risk & Shortage Interceptions
          </h3>
              ...
              >
                <AlertTriangle
                  className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                    alt.severity === 'Critical' ? 'text-slate-900' : 'text-indigo-600'
                  }`}
                />

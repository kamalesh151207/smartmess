...
export const InventoryPage: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="p-6 rounded-3xl glass-panel border border-slate-600 shadow-[0_0_30px_-5px_rgba(225,29,72,0.15)] space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-black text-slate-50 border border-blue-200">
              SUFFICIENCY AUDIT FOR TODAY'S FORECAST
            </span>
            <span className="text-xs text-slate-400">Target: 415 Lunch & 400 Dinner Meals</span>
          </div>
          <span className="text-xs font-bold text-slate-50">2 Items Require Immediate Purchase Alert</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-1">
          ...
          <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-50 block">Basmati Rice</span>
              <span className="text-[11px] text-slate-400">Stock: 380 kg • Demand: 85 kg</span>
            </div>
            <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-blue-100 text-slate-50 border border-blue-200">
              🟢 Sufficient
            </span>
          </div>
          ...
          <div className="p-3 rounded-xl bg-black border border-slate-600 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-50 block">Fresh Paneer</span>
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-slate-900 hover:bg-slate-950 text-slate-50 text-[11px] font-bold transition-all cursor-pointer"
            >
          ...
          <div className="p-3 rounded-xl bg-indigo-50 border border-indigo-300 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-50 block">Fresh Milk</span>
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-indigo-500 hover:bg-amber-400 text-slate-50 text-[11px] font-bold transition-all cursor-pointer"
            >
      ...
      {/* FILTER & SEARCH BAR */}
      <motion.div variants={itemVariants} className="p-4 rounded-3xl glass-panel border border-slate-800 shadow-lg flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
        {/* Search */}
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-xs text-slate-200 focus:outline-none focus:border-indigo-400"
          />
        </div>
        ...
        {categories.map((cat) => (
          <button
            ...
            className={`px-3 py-1 rounded-lg font-medium transition-colors cursor-pointer ${
              filterCategory === cat
                ? 'bg-slate-800 text-slate-50 border border-slate-700/40'
                : 'text-slate-400 hover:text-slate-50 bg-slate-900/60 border border-slate-800'
            }`}
          >
            {cat}
          </button>
      ...
      <motion.div variants={itemVariants} className="rounded-3xl glass-panel border border-slate-800 shadow-xl overflow-hidden">
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="py-3 px-4 font-bold text-slate-50">{item.item_name}</td>
                    <td className="py-3 px-4 text-slate-400">{item.category}</td>
                    <td className="py-3 px-4 text-center font-mono font-bold text-slate-50">
                    ...
                    <td className="py-3 px-4 text-center font-mono font-bold">
                      {item.recommended_purchase > 0 ? (
                        <span className="text-slate-50">+{item.recommended_purchase} {item.unit}</span>
                      ) : (
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {isShortage && (
                          <button
                            ...
                            className="px-2.5 py-1 rounded-md bg-blue-100 hover:bg-blue-100 text-slate-50 border border-slate-600 text-[11px] font-bold cursor-pointer transition-colors"
                          >
                        ...
                        <button
                          ...
                          className="p-1 text-slate-400 hover:text-slate-50 hover:bg-slate-800 rounded-md transition-colors cursor-pointer"
                          title="Edit Stock"
                        >
      ...
      {selectedItem && (
        ...
        >
          ...
          >
            ...
            <h3 className="text-lg font-bold text-slate-50 mb-1">Adjust Storage Stock</h3>
            <p className="text-xs text-slate-400 mb-4">{selectedItem.item_name}</p>
            ...
            <form onSubmit={handleUpdateStock} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-200 font-mono text-base"
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-xl bg-slate-950 hover:bg-slate-800 text-slate-50 font-bold text-sm transition-all cursor-pointer mt-2 shadow-[0_0_20px_-5px_rgba(16,185,129,0.4)]"
              >
